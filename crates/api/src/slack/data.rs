...
pub async fn threads(
    ...
    // Group by (channel, thread_ts), preserving chronological order.
...
pub async fn reply(
    ...
    // Echo locally; channel name + ticket key are inherited from the thread.
    let thread_rows = messages::list_for_ticket_thread(&state.db, user_id, channel_id, thread_ts)
        .await?;
    ...
    let posted_at: sea_orm::prelude::DateTimeWithTimeZone = chrono::Utc::now().into();
    let bot_user_id = row.bot_user_id.clone().unwrap_or_default();
    ...
    Ok(summary::SlackMessage {
        ts: posted.ts,
        author: summary::SlackAuthor {
            id: row.bot_user_id.unwrap_or_default(),
