...
async fn main() -> Result<()> {
    ...
    probe_dispatch(&token, &r).await;
    probe_merge(&token, &r).await;
    Ok(())
}
...
/// Pick the first selected repo (`owner/repo`) to probe against.
...
/// Dispatch a bogus workflow id; GitHub rejects (404) without creating anything.
async fn probe_dispatch(token: &str, r: &RepoRef) {
...
/// Merge a bogus PR number; GitHub rejects (404/422) without creating anything.
async fn probe_merge(token: &str, r: &RepoRef) {
